...
export default function HomeScreen() {
  ...
  const { history, lists, favorites, profile } = useData();

  const recentScans = history.slice(0, 6);
  const totalScanned = history.length;
  const favCount = favorites.length;
  const safeCount = history.filter(
    (p) => p.nutri_score === "A" || p.nutri_score === "B",
  ).length;
  const ecoCount = history.filter(
    (p) => p.eco_score === "A" || p.eco_score === "B",
  ).length;
        ...
        <View style={{ paddingTop: insets.top + 16 }}>
          <View style={styles.headerRow}>
            <View>
              ...
              <Text style={styles.title}>Open Product Facts</Text>
            </View>
            <Pressable
